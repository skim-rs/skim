complete -c sk -s t -l tiebreak -d 'Comma-separated list of sort criteria to apply when the scores are tied.' -r -f -a "{score\t'',begin\t'',end\t'',-score\t'',-begin\t'',-end\t'',length\t'',-length\t''}"
...
complete -c sk -l algo -d 'Fuzzy matching algorithm' -r -f -a "{skim_v1\t'',skim_v2\t'',clangd\t''}"
complete -c sk -l case -d 'Case sensitivity' -r -f -a "{respect\t'',ignore\t'',smart\t''}"
...
complete -c sk -l layout -d 'Set layout' -r -f -a "{default\t'',reverse\t'',reverse-list\t''}"
...
complete -c sk -l expect -d 'Comma separated list of keys used to complete skim' -r
...
complete -c sk -l inline-info -d 'Display info next to the query'
...
complete -c sk -l info -d 'Reserved for later use'
