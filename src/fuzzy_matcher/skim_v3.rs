...
//! - Try capping the number of allowed transpositions and/or deletions in the prefilter
...
const CELL_ZERO: Cell = Cell(Dir::None as u32);

impl Cell {
    #[inline(always)]
    pub fn new(score: Score, dir: Dir) -> Cell {
...
fn batch_score_only_bytes_simd<const ALLOW_TYPOS: bool>(
    ...
    for j in 1..=max_m {
        ...
        for i in 1..=n {
            ...
            let match_score = diag_score + match_bonus_v + bonus;
        ...
        if n > COLMAJOR_MAX_N {
            return items
                ...
                .map(|cho| {
                    ...
                    precompute_bonuses(cho, &mut bonus_buf);
                    let result = if self.allow_typos {
                        score_only_colmajor::<true, _>(cho, pattern, &bonus_buf, respect_case)
                    } else {
                        score_only_colmajor::<false, _>(cho, pattern, &bonus_buf, respect_case)
                    };
        ...
        while idx < items.len() {
            ...
            for lane in 0..batch_count {
                ...
                let pass = if self.allow_typos {
                    cheap_typo_prefilter(pattern, cho, respect_case)
                } else {
                    is_subsequence(pattern, cho, respect_case)
                };
                ...
                precompute_bonuses(cho, &mut bonus_storage[lane]);
            }
