...
impl Query {
    ...
    fn parse_options(&mut self, options: &SkimOptions) {
        ...
        if let Some(history_file) = &options.history {
            self.fz_query_history_before = read_file_lines(history_file)
                .unwrap_or_else(|_| panic!("Failed to open history file {}", history_file));
        }

        if let Some(cmd_history_file) = &options.cmd_history {
            self.cmd_history_before = read_file_lines(cmd_history_file)
                .unwrap_or_else(|_| panic!("Failed to open command history file {}", cmd_history_file));
        }
    }
